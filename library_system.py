...
# Library System
class LibrarySystem:
    PENALTY_PER_DAY = 1  # Penalty per day for late return
    ...
    def add_book(self, title, author, isbn, category):
        ...
        print(f"Book '{title}' added successfully with category '{category}'.")
    ...
    def borrow_book(self, isbn):
        ...
        for book in self.books:
            if book.isbn == isbn and not book.borrower:
                ...
                book.due_date = datetime.datetime.now() + datetime.timedelta(days=14)  # Borrowing period of 14 days
                self.logged_in_member.borrowed_books.append(book)
                print(f"Book '{book.title}' borrowed successfully. Due date: {book.due_date}.")
                return True
        ...
        for book in self.logged_in_member.borrowed_books:
            if book.isbn == isbn:
                ...
                if datetime.datetime.now() > book.due_date:
                    ...
                    print(f"Book '{book.title}' returned late. Penalty: ${penalty}.")
                else:
    ...
    def view_available_books(self):
        ...
        for book in available_books:
            print(f"Title: {book.title}, Author: {book.author}, ISBN: {book.isbn}, Category: {book.category}")
    ...
    def view_borrowed_books(self):
        ...
        for book in self.logged_in_member.borrowed_books:
            print(f"Title: {book.title}, Author: {book.author}, ISBN: {book.isbn}, Date Borrowed: {book.date_borrowed}, Due Date: {book.due_date}")
